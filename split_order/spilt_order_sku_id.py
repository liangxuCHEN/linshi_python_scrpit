...
def main_process(order_list):
    ...
    for order in order_list:
        ...
        if len(input_rows) > 3000:
            print('output data to db')
            # 导入到临时数据库
            output_data_sql(input_rows)
            input_rows = []
            print('output 3000 rows, pls wait.....')

if __name__ == '__main__':
    log = log_init('process_v3.log')
    # combine product
    ...
    print('initiate the table class....')
    init_connection(tmp_table_name)

    input_rows = []
    end_day = dt.today()
    # begin_day = None   dt.strptime('2017-03-01', '%Y-%m-%d')
    print('connect to the DB and get the data....')
    if args.begin_date:
        delta_days = (end_day-args.begin_date).days
        if delta_days <= 0:
            print('Begin day must be before today and programme is end now')
            # 有错误退出
            exit(1)
        else:
